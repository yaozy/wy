flyingon.widget({
    ...
    created: function () {
        ...
        grid.on('change', function (event) {
            ...
            if (event.target.column.name() === 'provincename')
            {
                dataset.uniqueId(event.target.row.id).set('cityname', cities[event.value][0]);
            }
        });

        grid.columns(5).onopening = function () {

            this.items(cities[this.row.data.provincename]);
        }
